...
int at_min_level(Heap* heap, int index){
    if(heap->min_max == 1 && get_tree_level(index) % 2 == 0) return 1;
    if(heap->min_max == 0 && get_tree_level(index) % 2 == 1) return 1;
    return 0;
}
...
int find_heap_max_index(Heap* heap){
    if(heap->size == 1 || heap->min_max == 0) return 0;
    if(heap->size == 2) return 1;
    else if(heap->arr[1] > heap->arr[2]) return 1;
    else return 2;
}

//returns the minimums's index of the given heap
int find_heap_min_index(Heap* heap){
    if(heap->size == 1 || heap->min_max == 1) return 0;
    if(heap->size == 2) return 1;
    else if(heap->arr[1] < heap->arr[2]) return 1;
    else return 2;
}
...
void insert(int element){
    if(pq.greater.size == 0){
        ...
        heap_insert(&pq.greater, element);
        if(pq.greater.size - pq.lesser.size == 2) heap_insert(&pq.lesser, pop_min(&pq.greater));
        
    }else{
        heap_insert(&pq.lesser, element);
        if(pq.lesser.size > pq.greater.size) heap_insert(&pq.greater, pop_max(&pq.lesser));
...
int find_min(){
    if(pq.lesser.size == 0) return pq.greater.arr[0];
    return pq.lesser.arr[find_heap_min_index(&pq.lesser)];
}

//Fetches but does not remove the median element.
int find_median(){
    if(pq.lesser.size == pq.greater.size) return pq.lesser.arr[0];
    else return pq.greater.arr[0];
}
...
int delete_median(){
    if(pq.lesser.size == pq.greater.size) return pop_max(&pq.lesser);
    else return pop_min(&pq.greater);
}

//Deletes and returns the maximum element.
int delete_max(){
    if(pq.greater.size == pq.lesser.size) heap_insert(&pq.greater, pop_max(&pq.lesser));
...
int main() {
    ...
    while(num_operations > 0){        
        ...
        if(operation == 'I'){ //Insert operation
            ...
            if(pq_size != 0){
                if(target == 'M') delete_min();
                else if(target == 'X') delete_max();
                else if(target == 'E') delete_median();
            }
            ...
            if(pq_size != 0){
                int found;
                if(target == 'M') found = find_min();
                else if(target == 'X') found = find_max();
                else if(target == 'E') found = find_median();
                printf("%d\n", found);
            }else{
                printf("NULL\n");
            }
